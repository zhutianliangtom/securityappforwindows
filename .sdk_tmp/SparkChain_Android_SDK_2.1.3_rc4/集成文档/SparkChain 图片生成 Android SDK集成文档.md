# SparkChain 图片生成 Android SDK集成文档

## 1. 图片生成简介

SparkChain支持用户向大模型发送一个文本请求，大模型根据请求生成相应的图片。
...
└── SparkChain 图片生成 Android SDK集成文档.pdf SparkChain集成指南
...
| 功能     | 说明                                                         |
| :------- | :----------------------------------------------------------- |
| 图片生成 | 构造方法： <br />public static LLM imageGeneration() public static LLM imageGeneration(LLMConfig config) <br />public static LLM imageGeneration(int width, int height)  <br />public static LLM imageGeneration(int width, int height, LLMConfig config) <br />参数说明： <br />config:星火大模型配置参数。其中的domain为tti，其他domain参数不生效。 <br />width:生成图片的宽度。参考下方分辨率说明, 不同的分辨率计费不同，请选择合适的使用 height:生成图片的高度。参考下方分辨率说明, 不同的分辨率计费不同，请选择合适的使用 |
...
- LLMConfig参数说明：
  ...
  | :---------- | :------------------------------------------ | :------- | :----------------------------------------------------------- | :------- |
  | domain      | 需要使用的领域                              | String   | 取值为[tti]，默认tti<br />tti: 图片生成访问的是大模型tti服务 <br />domain的取值对应的url不同，需要严格对应。url地址参见下文。 | 否       |
  | url         | 配置chat服务器域名地址                      | String   | tti:https://spark-api.cn-huabei-1.xf-yun.com/v2.1/tti<br />url和domain是配合使用的，SDK里预设了tti的url。当使用这几个domain时，SDK会自动设置url，故开发者可不用额外设置其值。SDK同样支持开发者访问其他未预置的服务，此时则需要开发者同时设置domain和url。 | 否       |
...
```java
LLMConfig tti_llmConfig = LLMConfig.builder()    
    .maxToken(2048)    
    ... 
    .topK(4);//config配置可缺省，demo仅供展示如何使用,开发者根据实际情况选择。 
LLM tti_llm = LLMFactory.imageGeneration(1024,1024,tti_llmConfig);
```

## 9. 同步调用

用户可以通过tti_llm.run方法向大模型发送图片生成请求，并获取大模型返回的结果，不支持单实例并发调用。调用接口如下：
...
| 参数名   | 类型   | 说明             | 限制                                         | 是否必填 |
| :------- | :----- | :--------------- | :------------------------------------------- | :------- |
| question | String | 文本内容         | 0-1000字符                                   | 是       |
...
用户可以通过tti_llm.arun方法向大模型发送图片生成请求，并从监听回调中一次性获取交互结果，不支持单实例并发调用。

### 10.1 注册结果监听回调

图片生成的异步请求结果通过LLMCallbacks监听回调返回，LLMCallbacks监听回调接口如下：
...
tti_llm.registerLLMCallbacks(mLLMCallbacksListener);
```
...
```java
String question = "帮我画一座山。"; 
int ret = tti_llm.arun(question); 
//带有用户自定义标识交互 
//String myContext = "myContext"; 
//int ret = tti_llm.arun(question,myContext);
```
