# SparkChain 实时语音转写大模型 Android SDK集成文档
...
└── SparkChain 实时语音转写大模型 Android SDK集成文档.pdf     SparkChain集成指南
...
**在使用SDK功能前，需要先开通实时语音转写大模型授权并获取已开通授权的应用信息（appId、apiKey、apiSecret）。SDK全局只需要初始化一次。**初始化时，开发者需要构建一个SparkChainConfig实例config，把相关的appid信息以及日志设置等传入config中，然后再通过SparkChain.getInst().init方法把config实例设置到SDK中。具体初始化示例如下：
...
在使用实时语音转写大模型功能前，需先通过其构造方法RTASR()方法构建其实例，然后用该实例调用相应的方法去设置转写参数。

实时语音转写大模型构造方法如下：

```Java
public class RTASR {
    public RTASR(String apiKey,RegionType regionType) {
        ...
    }
}
```
...
```Java
RTASR mRTASR = new RTASR("",RegionType.CN_LLM_TYPE);//RegionType.CN_LLM_TYPE（实时语音转写大模型）
```
...
| ------------- | ------------- | -------- | ------------------------------------------------------------ | -------- | --------------- |
| lang          | lang          | String   | 可选范围：<br/>autodialect：支持中英 + 202 种方言免切识别<br/>autominor：支持 37 个语种免切识别 （暂需联系人工对接） | 是       | 如：autodialect |
| audio_encode  | audio_encode  | String   | 1. pcm 格式，传参是 `pcm_s16le`，代表 16k16bit 原始音频流<br/>2. speex 格式，支持 `speex-7`、`speex-10`<br/>3. opus 格式（推荐），传参为 `opus-wb` | 是       | 如：pcm_s16le   |
| samplerate    | samplerate    | long     | 采样率，仅在客户端音频为 pcm 时必须传入，支持 16000、8000    | 是       | 如：16000       |
| role_type     | role_type     | short    | 是否开启说话人分离：<br /> 0：关闭说话人分离（默认） <br /> 2：开启实时角色分离（盲分模式）可配合注册声纹使用 | 否       |                 |
| feature_ids   | feature_ids   | string   | 声纹Id列表.以英文逗号分隔拼接。【通过[注册声纹 ](https://www.xfyun.cn/doc/spark/asr_llm/voice_print.html)模式分离发音人】<br/>注意：必须开启说话人分离(role_type=2)才有作用. | 否       |                 |
| eng_spk_match | eng_spk_match | int      | 使用注册声纹模式下(请求参数携带 feature_ids ),控制角色分离结果角色信息全部来自声纹库角色，不传参默认关闭<br/>开 1 ;关: 0 | 否       |                 |
| pd            | pd            | string   | 领域个性化参数，优化特定领域识别效果：<br/>法律：court、<br/>金融：finance、<br/>医疗：medical、<br/>科技：tech、<br/>体育：sport、<br/>教育：edu、<br/>运营商：isp、<br/>政府：gov、<br/>游戏：game、<br/>电商：ecom、<br/>军事：mil、<br/>企业：com、<br/>生活：life、<br/>娱乐：ent、<br/>人文历史：culture、<br/>汽车：car | 否       |                 |
| eng_punc      | eng_punc      | string   | 标点过滤控制字段，默认返回标点，`punc=0` 会过滤结果中的标点  | 否       |                 |
| eng_vad_mdn   | eng_vad_mdn   | int      | vad 远近场切换： 不传或传 1 代表远场，传 2 代表近场          | 否       |                 |

具体配置示例如下：

```Java
mRTASR.setParams("lang", "autodialect");
...
实时语音转写大模型运行结果通过RTASRCallbacks异步返回，接口定义如下：
...
- onResult为实时语音转写大模型结果回调方法，参数说明如下：
...
| void           | public void registerCallbacks(RTASRCallbacks cbs)<br />注册实时语音转写大模型的结果监听回调 |
