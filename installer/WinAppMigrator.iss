...
; --- 准备安装页 ---
ReadyLabel2a=📋 万事俱备，只欠东风%n（东风 = 你点一下「安装」按钮）%n%n按下安装后，我们会把 zhuzhu Copilot 安置妥当，%n从此你的 C 盘将获得新生 🌱🌱🌱%n%n⚠️ 免责声明：%n本工具不负责安抚被迁移应用的思乡情绪。%n它们在新家过得好不好，取决于新盘符的风水 🧧
ReadyLabel2b=📋 万事俱备，只欠东风%n（东风 = 你点一下「安装」按钮）%n%n按下安装后，我们会把 zhuzhu Copilot 安置妥当，%n从此你的 C 盘将获得新生 🌱🌱🌱%n%n⚠️ 免责声明：%n本工具不负责安抚被迁移应用的思乡情绪。%n它们在新家过得好不好，取决于新盘符的风水 🧧

; --- 完成页 ---
FinishedLabel=🎉 🎉 🎉  恭喜！安装完成  🎉 🎉 🎉%n%n⚔️ zhuzhu Copilot 已就位，随时待命。%n%n现在打开它，开始拯救你的 C 盘吧，勇士！%n%n%n📌 记住：%n  🔴 红色的 C 盘 = 病，得治。%n  💊 本工具 = 你的处方药。%n%n      C 盘：谢谢你... 🥹

; --- 卸载确认 ---
ConfirmUninstall=🥺 真的要卸载吗？%n%n你的 C 盘可能会想念这个救星的...%n%n（当然，卸载后应用们不会搬回去，%n它们已经在新家安居乐业了 🏡）

; --- 已存在版本 ---
SetupAppRunningError=🤯 检测到 zhuzhu Copilot 正在运行！%n%n请先关闭它再安装，我们暂时不支持平行宇宙。
...
[Run]
Filename: "{app}\{#MyAppExeName}"; Description: "🚀 立即启动 zhuzhu Copilot（C 盘在哭泣，快去吧！）"; Flags: nowait postinstall skipifsilent
...
begin
  { 欢迎页 — 超大标题 }
  WizardForm.WelcomeLabel1.Caption := '🛟 欢迎使用 zhuzhu Copilot 🛟';
