...
- 只有**无文字的纯图标/图形目标**才用坐标：`move_mouse(x,y)` 移动 → `screenshot` 看红色准星是否套住 →
  未对准按偏移修正 → 对准后 `click(x,y)`；目标太小先 `zoom_in(x,y)` 放大。
...
3. 创建成功后提示：已可通过 /技能名 或对话描述调用；若用户描述的是可复用的流程，适合沉淀为技能。""",
    ...
    "computer-control": {
        "description": "操控电脑：AI 用鼠标/键盘接管电脑完成任务的标准流程，强调点击准确率、执行速度与质量，并响应驱动AI操控停止",
        "instruction": """# computer-control：AI 操控电脑（标准流程）

当 AI 需要接管鼠标/键盘替用户操作电脑完成任务时使用本技能。目标是"用户无干预也能可靠完成"，
因此每一步都以**准确率、速度、质量**为基准，并遵守用户随时接管机制。

## 0. 接管协议（强制）
- 当前处于"AI 操控中"状态（屏幕中下部有字幕气泡）时，用户可随时手动移动鼠标/按键/点击；
  一旦检测到用户手动操作，系统会**立即停止 AI 操控**并交还控制权。
- AI 每次调用鼠标/键盘工具前，先通过字幕说明当前操作，让用户知道 AI 在做什么、是否要接管。
- 若上一轮被用户接管中止，不得强行继续，先向用户说明进展并等待指令。

## 1. 先观察再动手
- 未知界面：先 screenshot 截屏，返回**可点击/输入元素的编号清单 [id] (类型) 文字**。
- 只操作特定窗口：list_windows 定位 → capture_window(window) 只截该窗口并返回其清单。

## 2. 定位（语义优先，准确率第一）
- **点击只按清单里的 [id] 或文字**：click(id=编号) / click_text(text) / click(text=目标文字)，
  系统 UIA 语义树+OCR 精确解析像素中心，100% 精准，无需也没必要自己估坐标。
- 纯图标/图形目标（无文字）：move_mouse 移动 → 截图看红色准星是否套住 → 未对准按偏移微调修正 → 对准后 click(x,y)。
- 目标过小：zoom_in(x, y) 放大后按细刻度读数，再以该坐标 click（系统自动换算，勿手动换算）。
- 坐标一律交给系统换算，禁止自行心算叠加误差。

## 2.5 反馈与验证（强制）
- 点击/输入不再自动返回整屏截图（提速）；需要核对界面时**主动** screenshot 或 refresh_screen。
- 每完成一步关键操作，screenshot 验证结果再继续下一步；没点中根据截图修正换方案，
  禁止盲目连点同一位置。
- 专属电脑操控 Agent 会自动：语义目标精确定位中心；坐标目标若落在文字控件上自动吸附纠偏；
  移动鼠标带真人轨迹并可视化。优先用 click(id=..) / click(text=..) 走最精准语义路径。

## 3. 执行（速度与质量平衡）
- 每完成一步有用信息的关键操作，截图验证结果再继续下一步，避免方向错误浪费多轮。
- 失败先自查（是否弹窗/焦点/坐标偏移），换方案重试；禁止盲目重复点击同一位置。
- 长流程任务：先列步骤，按先后顺序逐步执行，每步验证通过再进入下一步。
- 键盘输入：type_text 输文本、press_key 发按键；剪贴板用 clipboard(action=read/write)。

## 4. 完成与汇报
- 目标达成后截图确认最终结果，输出：完成的操作、关键步骤结果、最终界面状态。""",
    },
}
...
DEFAULT_AGENTS = [
    {"name": "zhuzhu Copilot", "description": "zhuzhu Copilot：观察屏幕并操控电脑高质量完成任务",
     ...
     "rules": [
         ...
         "4. **点击/输入只按 screenshot/refresh_screen 返回的编号清单 [id] 或文字**："
         "click(id=编号) / click_text(text) / click(text=目标文字)，系统 UIA 语义树+OCR 精确解析像素中心，"
         "100% 精准，无需读坐标。只有无文字的纯图标/图形目标才用准星对齐：move_mouse 移动鼠标 → "
         "截图看红色准星是否套住目标 → 未对准按准星与目标的偏移修正坐标，对准后 click(x,y)（系统自动换算真实屏幕坐标）；"
         "目标太小先 zoom_in 放大。禁止自己心算坐标。",
         ...
         "9. 精确点击：目标是按钮/菜单/对话框等**文字类元素**时，优先用 click(id=编号) 或 click_text"
         "按清单/文字精确定位（系统 UIA 语义树+OCR 自动找文字像素中心，无需估算坐标）；"
         "纯图标/图形无文字时，用准星对齐法（见规则 4）：先 move_mouse 粗定位，"
         "截图看准星，未对准修正坐标再 move_mouse，对准后 click(x,y)。"
     ...
     "tool_instructions": {
         "screenshot": {
             "理解": "截取当前目标窗口（AI 视觉模型直接看到截图），同时返回可点击/输入元素的编号语义清单 "
                    "（如 [3] (按钮) 确定），清单坐标由系统解析，按 [id] 或文字用 click/click_text 点击即可，无需读坐标。"
                    "截图上的红色准星=当前鼠标位置，仅用于无文字的图标目标对齐。",
             "执行拆分": ["需要了解当前界面/验证结果时截图",
                          "界面变化后先用 refresh_screen 刷新清单更省时",
         ...
         "type_text": {
             "理解": "向目标输入框输入文本（支持中文），可含 enter/tab 等特殊键。"
                    "用 id=输入框编号 或 target=输入框文字 先自动点击定位再输入，最稳。",
             "执行拆分": ["用 id/target 指定目标输入框（先聚焦再输入）",
                          "输入后按需截图确认内容正确",
                          "需换行/提交时用 enter"],
         },
         "find_app": {
...
def build_system_prompt(agent_name: str = "", extra_skills: list = None,
    ...
    prompt += ("\n\n可用内置工具：screenshot(截目标窗口并返回元素编号清单)、refresh_screen(界面变化后刷新清单)、"
               "list_windows/capture_window(枚举并只截指定窗口，避免其他窗口干扰)、"
               "click(id/text 按清单或文字精确点击)、click_text(按文字点击)、type_text(target/id 输入)、"
               "get_screen_size(分辨率)、"
