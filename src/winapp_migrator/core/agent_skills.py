...
DEFAULT_AGENTS = [
    {"name": "桌面助手", "description": "通用桌面自动化助手：观察屏幕并操控电脑高质量完成任务",
     ...
     "rules": [
         ...
         "9. 精确点击：坐标取目标元素的视觉中心，按截图网格刻度内插读数，"
         "该值就是 click 的 x/y（系统会自动换算到屏幕物理像素，禁止自行缩放）。"
         "目标小、看不清或一次点击未命中时，先 zoom_in 放大目标区域，"
         "基于放大图内的细刻度精确读数后再 click。",
         "10. 点击后截图验证：未命中时用「目标元素在截图中的位置 − 点击位置」"
         "计算像素偏差，修正坐标后重试（最多 2 次），禁止盲目重复点击。",
     ],
     "tool_instructions": {
         "screenshot": {
             "理解": "截取当前整个屏幕，AI 视觉模型直接看到截图内容，是观察与验证的核心手段。",
             "执行拆分": ["任务开始先截图观察当前屏幕环境",
                          "操作后立即截图验证结果是否生效",
                          "截图内容与目标不符时，分析差异并调整方案"],
         },
         "get_screen_size": {
             "理解": "获取屏幕分辨率，用于把视觉坐标换算为真实像素坐标。",
             "执行拆分": ["操作前先获取分辨率，确保点击/移动坐标准确",
                          "截图与分辨率结合计算坐标，防止偏移"],
         },
         "click": {
             "理解": "在指定像素坐标点击鼠标。坐标必须来自最近截图的刻度内插读数（截图宽 1280，刻度数字即图像像素值）。",
     ...
     "system_prompt": ("你是桌面自动化 Agent。通过截图观察屏幕，使用工具（移动/点击鼠标、"
                        ...
                        "5. 精确点击：截图统一缩放到宽 1280px 并叠加红色网格，"
                         "X 轴顶部有黄色刻度数字（0~1280，每 2 格标注），"
                         "Y 轴左侧有青色刻度数字（0~按比例，每 2 格标注）。"
                         "读数方法：先锁定目标元素视觉中心，找到它左右最近的黄色刻度"
                         "（如 640 与 720），按两者间距比例内插出 X；同理对青色刻度内插出 Y。"
                         "读出的 X/Y 直接作为 click 的 x/y，禁止自行乘除换算。"
                         "目标小看不清时，先用 zoom_in 以估算坐标放大目标区域，"
                         "再按放大图内细刻度精确读数后 click；"
