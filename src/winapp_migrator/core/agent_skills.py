...
## 2. 点击（按优先级）
- 文字类目标（按钮/菜单/输入框/链接）：**click(text=目标文字)** 或 click_text(text)，
  系统 UIA+OCR 自动定位文字像素中心，100% 精准，无需自己估算坐标
- 图标/图形目标：move_mouse 移动 → 截图看红色准星是否套住目标 → 未对准按偏移修正坐标 → 对准后 click
- 目标太小：zoom_in(x, y) 放大后按细刻度读数，再以该坐标 click（系统自动换算）
- 点击后工具会返回反馈截图（含鼠标准星），用其核对是否点中，未点中修正重试
...
## 1. 先观察再动手
- 未知界面：先 screenshot 截屏，看清按钮/输入框/菜单布局再操作。
- 只操作特定窗口：list_windows 定位 → capture_window(window) 只截该窗口，避开遮挡干扰。

## 2. 定位（准确率第一）
- 文字类目标（按钮/菜单/输入框/链接）：**优先用 click(text=目标文字)** 或 click_text(text)，
  系统 UIA+OCR 自动定位文字像素中心，100% 精准，无需自己估算坐标。
- 纯图标/图形目标：move_mouse 移动 → 截图看红色准星是否套住目标 → 未对准按偏移微调坐标修正 → 对准后点击当前指针。
- 目标过小：zoom_in(x, y) 放大后按细刻度读数，再以该坐标 click（系统自动换算坐标，勿手动换算）。
- 坐标一律交给系统换算，禁止自行心算叠加误差。

## 2.5 点击反馈（强制）
- 每次 click / click_text 后，工具会自动返回一张当前屏幕截图（含红色准星=当前鼠标落点）。
- 你必须用这张反馈截图核对点击是否落在目标上：点中了继续下一步；
  没点中根据准星与目标的偏移修正坐标/换文字再点，禁止盲目连点同一位置。
- 专属电脑操控 Agent 会自动：文字目标精确定位中心；坐标目标若落在文字控件上自动吸附纠偏；
  移动鼠标带真人轨迹并可视化。优先用 click(text=目标文字) 走最精准路径。
