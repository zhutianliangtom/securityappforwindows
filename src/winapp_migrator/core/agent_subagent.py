...
# ---- 电脑操控专用子 Agent ----
# 主 Agent 不直接摸键鼠/截图，而是通过 control_ui 派发目标，由本子 Agent 用
# UI_CONTROL_TOOLS 完成 GUI 操作（语义树优先：按 [id]/文字精确点击输入）。
_UI_SYSTEM = """你是「电脑操控专用子 Agent」，负责用鼠标/键盘完成主 Agent 交给的 GUI 操作目标。
规则：
1. 先 screenshot 截取目标窗口，拿到可点击/输入元素的编号清单 [id] (类型) 文字。
2. 点击/输入**只按清单里的 [id] 或文字**（click(id=..) / click_text(text=..) / type_text 的 id/target），
   系统精确解析像素，禁止自己估坐标/读刻度。
3. 只有无文字的纯图标目标才用 move_mouse + click(x,y) 准星对齐。
4. 每步操作前用一句话说明意图；界面变化后先 refresh_screen 刷新清单；结果不确定时 screenshot 验证。
5. 完成目标后输出精炼总结：做了什么、结果如何、最终界面状态；无法完成时说明已尝试的范围与原因。"""


def run_ui_agent(llm, goal, stop=None, on_status=None) -> str:
    """运行电脑操控专用子 Agent：用鼠标/键盘完成 GUI 操作目标，返回结果总结。

    只暴露 UI_CONTROL_TOOLS；语义树优先，目标窗口由 agent_locator 粘性目标确定。
    不做轮数限制，上下文过长由 _compress 压缩并保留任务目标。
    """
    goal = (goal or "").strip()
    if not goal:
        return "（空任务）"
    from winapp_migrator.core import agent_tools as _at
    tools = [t for t in _at.tool_schemas()
             if t["function"]["name"] in _at.UI_CONTROL_TOOLS]
    messages = [{"role": "system", "content": _UI_SYSTEM},
                {"role": "user", "content": agent_llm.build_content(goal)}]
    while True:
        if stop and stop():
            return "（电脑操控已停止）"
        try:
            res = llm.chat_stream(messages, tools=tools, tool_choice="auto", stop=stop)
        except agent_llm.AgentLLMError as e:
            return f"电脑操控子 Agent 调用失败: {e}"
        calls = res["tool_calls"]
        if not calls:
            return res["text"] or "（电脑操控完成）"
        ...
        for c in calls:
            ...
            if on_status:
                on_status(f"待执行工具: {name}")   # 面板据此显示操作字幕
            try:
                ...
                messages.append({"role": "tool", "tool_call_id": c["id"],
                                 "content": ("[工具参数错误] tool_calls.arguments 不是合法 JSON，"
                                             "请检查参数格式并重新发起该工具调用。")})
                continue
            if name not in _at.UI_CONTROL_TOOLS:
                messages.append({"role": "tool", "tool_call_id": c["id"],
                                 "content": f"[沙盒] 电脑操控子 Agent 只允许操控工具，已拒绝调用 {name}"})
                continue
            try:
                text = _at.execute_tool(name, args).get("text", "")
